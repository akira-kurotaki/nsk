        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (NskAppContext dbContext = new(dbInfo.ConnectionString, dbInfo.DefaultSchema, ConfigUtil.GetInt(Core.CoreConst.COMMAND_TIMEOUT)))
                {
                    ...
                    // ６．帳票を作成する。
                    using (var reportController = new NskReportP107100Controller(dbInfo))
                    {
                        ControllerResult controllerResult = null;
                        // ６．１．「帳票制御処理設計書_NSK_107100P_未納者リスト」を呼び出す。
                        controllerResult = reportController.ManageReports("NSK_107100P", jid, todofukenCd, kumiaitoCd, shishoCd, nBid);

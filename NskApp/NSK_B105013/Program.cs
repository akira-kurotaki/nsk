...
namespace NSK_B105013
{
    ...
    class Program
    {
                        ...
                        using (StreamWriter writer = new(fs, encoding))
                        {
                            ...
                            List<string> haniRecords =
                                [
                                    ...
                                    Constants.GIS_DATA_OUTPUT_TYPE_KOTEITYO
                                ];
                            // 配列の内容を書き込む
        ...
        private static List<SaimokuDataRecord> GetSaimokuData(NskAppContext dbContext, string todofukenCd, string kumiaitoCd, BatchJoken batchJoken)
        {
            ...
            sql.Append($@"    T4.rs区分, ");
            sql.Append($@"    concat( ");
            sql.Append($@"        lpad( T4.局都道府県コード, 4, '0'), ");
            sql.Append($@"        lpad( T4.市区町村コード, 3, '0'), ");
            sql.Append($@"        lpad( T4.大字コード, 8, '0'), ");
            sql.Append($@"        lpad( T4.小字コード, 4, '0'), ");
            sql.Append($@"        lpad( T4.地番, 16, '0'), ");
            sql.Append($@"        lpad( T4.枝番, 14, '0'), ");
            sql.Append($@"        lpad( T4.子番, 10, '0'), ");
            sql.Append($@"        lpad( T4.孫番, 10, '0')) ");
            sql.Append($@"    as GISデータ, ");
            ...
            sql.Append($@"    INNER JOIN t_11100_引受gis T4 ");
            sql.Append($@"        ON T4.組合等コード = T1.組合等コード ");
            sql.Append($@"        AND T4.年産 = T1.年産 ");
            sql.Append($@"        AND T4.共済目的コード = T1.共済目的コード ");
            sql.Append($@"        AND T4.組合員等コード = T1.組合員等コード ");
            sql.Append($@"        AND T4.耕地番号 = T1.耕地番号 ");
            sql.Append($@"        AND T4.分筆番号 = T1.分筆番号 ");
            sql.Append($@"WHERE ");
            sql.Append($@"      1 = 1 ");

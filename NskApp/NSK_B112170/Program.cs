...
namespace NSK_B112170
{
    ...
    class Program
    {
        ...
        /// バッチ名
        /// </summary>
        private static string BATCH_NAME = "加入申込書（細目・カンマ）大量データ受入";
        private static string BATCH_USER_NAME = "NSK_112170B";
        ...
        static void Main(string[] args)
        {
            ...
            var ukeireOkList = new List<T19070大量データ受入加入申込書ok>();
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    {
                        logger.Error("ERROR: データスタートレコードが存在しません。");
                        errorMsg = "ERROR: データスタートレコードが存在しません。";
                        goto forcedShutdown;
                    }
                    ...
                    for (int i = 3; i < allLines.Length; i++)
                    {
                        ...
                        string 年産 = dataFields[21].Trim().Trim('"');
                        string 実量基準単収 = (dataFields.Length > 22) ? dataFields[22].Trim().Trim('"') : "";
                        string 統計市町村コード = (dataFields.Length > 23) ? dataFields[23].Trim().Trim('"') : "";
                        string 統計単位地域コード = (dataFields.Length > 24) ? dataFields[24].Trim().Trim('"') : "";
                        string 統計単収 = (dataFields.Length > 25) ? dataFields[25].Trim().Trim('"') : "";
                        string 用途区分 = (dataFields.Length > 26) ? dataFields[26].Trim().Trim('"') : "";
                        string 産地別銘柄コード = (dataFields.Length > 27) ? dataFields[27].Trim().Trim('"') : "";
                        string 受委託者コード = (dataFields.Length > 28) ? dataFields[28].Trim().Trim('"') : "";
                        ...
                        {
                            dataError = MessageUtil.Get("ME10005", "田畑区分");
                            AddError(ukeireRirekiId, errListSeq++, 行番号, dataError, sysDate, db);
                            エラー件数++;
                            continue;
                        }
                        ...
                        if (0 == GetYoutoKbnCount(db, 共済目的コード, 用途区分))
                        {
                            dataError = MessageUtil.Get("ME10005", "用途区分");
                            AddError(ukeireRirekiId, errListSeq++, 行番号, dataError, sysDate, db);
                            エラー件数++;
                            continue;
                        }
                        ...
                        if (0 == GetSanchiBetuMeigaraCount(db, kumiaitoCd, nensan, 共済目的コード, 産地別銘柄コード))
                        {
                            dataError = MessageUtil.Get("ME10005", "産地別銘柄コード");
                            AddError(ukeireRirekiId, errListSeq++, 行番号, dataError, sysDate, db);
                            エラー件数++;
                            continue;
                        }
                        ...
                        if (0 == GetToukeitaniChiikiCount(db, kumiaitoCd, nensan, 共済目的コード, 統計単位地域コード))
                        {
                            dataError = MessageUtil.Get("ME10005", "統計単位地域コード");
                            AddError(ukeireRirekiId, errListSeq++, 行番号, dataError, sysDate, db);
                            エラー件数++;
                            continue;
                        }
                        ...
                        {
                            dataError = MessageUtil.Get("ME10011", "引受面積", "転作等");
                            AddError(ukeireRirekiId, errListSeq++, 行番号, dataError, sysDate, db);
                            エラー件数++;
                            continue;
                        }
                        ...
                        List<T11010個人設定> shuruiMeishoList = GetShuruiMeishouRecords(db, kumiaitoCd, nensan, 共済目的コード, 組合員等コード);
                        bool gotoFlg = false;
                        ...
                        // 受入OKデータ作成
                        var dataOkList = new T19070大量データ受入加入申込書ok
                        {
                            ...
                            共済目的コード = dataFields[0].Trim().Trim('"'),
                            組合員等コード = dataFields[1].Trim().Trim('"'),
                            耕地番号 = dataFields[2].Trim().Trim('"'),
                            分筆番号 = dataFields[3].Trim().Trim('"'),
                            類区分 = GetField(dataFields, 4).Trim().Trim('"'),
                            地名地番 = GetField(dataFields, 5).Trim().Trim('"'),
                            耕地面積 = decimal.Parse(GetField(dataFields, 6).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            引受面積 = decimal.Parse(GetField(dataFields, 7).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            転作等面積 = decimal.Parse(GetField(dataFields, 8).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            受委託区分 = GetField(dataFields, 9).Trim().Trim('"'),
                            備考 = GetField(dataFields, 10).Trim().Trim('"'),
                            田畑区分 = GetField(dataFields, 11).Trim().Trim('"'),
                            区分コード = GetField(dataFields, 12).Trim().Trim('"'),
                            種類コード = GetField(dataFields, 13).Trim().Trim('"'),
                            品種コード = GetField(dataFields, 14).Trim().Trim('"'),
                            収量等級コード = GetField(dataFields, 15).Trim().Trim('"'),
                            参酌コード = GetField(dataFields, 16).Trim().Trim('"'),
                            基準単収 = decimal.Parse(GetField(dataFields, 17).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            基準収穫量 = decimal.Parse(GetField(dataFields, 18).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            修正日付 = DateTime.Parse(GetField(dataFields, 19).Trim().Trim('"')),
                            計算日付 = DateTime.Parse(GetField(dataFields, 20).Trim().Trim('"')),
                            年産 = short.Parse(dataFields[21].Trim().Trim('"')),
                            実量基準単収 = decimal.Parse(GetField(dataFields, 22).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            統計市町村コード = GetField(dataFields, 23).Trim().Trim('"'),
                            統計単位地域コード = GetField(dataFields, 24).Trim().Trim('"'),
                            統計単収 = decimal.Parse(GetField(dataFields, 25).Trim().Trim('"'), CultureInfo.InvariantCulture),
                            用途区分 = GetField(dataFields, 26).Trim().Trim('"'),
                            産地別銘柄コード = GetField(dataFields, 27).Trim().Trim('"'),
                            受委託者コード = GetField(dataFields, 28).Trim().Trim('"'),
                            登録日時 = sysDate,
                            登録ユーザid = BATCH_USER_NAME,
                        };

                        // DBへ追加して保存
                        db.T19070大量データ受入加入申込書oks.Add(dataOkList);
                        db.SaveChanges();
                        OK件数++;
                    } // end for
                    // トランザクションコミット
                    ...
                    if (エラー件数 > 0)
                    {
                        ...
                        List<T01080大量データ受入エラーリスト> getErrorListByTorikomiId = GetErrorListByTorikomiId(db);
        ...
        // 2. 大量データ受入履歴テーブルから受入履歴IDに一致するレコード取得
        ...
        // 12. m_10140_種類名称テーブルから件数取得（種類コード指定）
        ...
        // 15. m_00130_産地別銘柄名称設定テーブルから件数取得
        ...
        // 19. t_19070_大量データ受入_加入申込書OKテーブルから件数取得
        private static int GetJoinApplicationOkCount(NskAppContext db, string kumiaitoCd, string nensan, string kyosaimokutekiCd, string kumiaiintoCd, string kouchiBango, string bunpituBango)
        {
        ...
        private static List<T01080大量データ受入エラーリスト> GetErrorListByTorikomiId(NskAppContext db)
        {
        ...
        // 23. m_00030_区分名称テーブルからレコード取得
        private static List<M00030区分名称> GetKubunMeishouRecords(NskAppContext db, string kumiaitoCd, string nensan, string kyosaimokuteCd, string kbnCd)
        {
        ...
        // 24. m_10140_種類名称テーブルからレコード取得
        private static List<T11010個人設定> GetShuruiMeishouRecords(
            NskAppContext db
            ,string kumiaitoCd
            ,string nensan
        ...
        private static string ValidateRangeRecord(string[] fields, int lineNumber)
        {
            ...
            string 抽出区分 = fields[3].Trim().Trim('"');
            ...
            switch (抽出区分)
            {
                case "1":
                    ...
                    // 3. 共済目的コード：半角数字、文字数2（入力があればチェック）
                    ...
                    // 6. 範囲パラメータ２：文字列、文字数2（入力があれば桁数チェック）
                    ...
                    // 7. 範囲パラメータ３：半角数字（入力があればチェック）
                    string field7 = fields[6].Trim().Trim('"');
                    if (!string.IsNullOrEmpty(field7) && !IsAllDigits(field7))
                    {
                ...
                case "3":
                    ...
                    // ※ 抽出区分=3 では、範囲パラメータ２存在する場合は s7（範囲パラメータ３）は必須
                    if (!string.IsNullOrEmpty(s6))
                    {
        ...
        private static string ValidateDataRow(string[] fields, int lineNumber)
        {
            ...
            // 7. 耕地面積（任意、存在する場合、半角数字、9文字／表示形式(9,1)）
            string str耕地面積 = GetField(fields, 6).Trim().Trim('"');
            string pattern91 = @"^\d{1,8}\.\d{1}$";
            if (!string.IsNullOrEmpty(str耕地面積))
            {
                ...
                if (!Regex.IsMatch(str耕地面積, pattern91))
                {
                    errMsg = MessageUtil.Get("ME00004", "耕地面積", "8", "1", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 8. 引受面積（任意、存在する場合、半角数字、9文字／表示形式(9,1)）
            ...
            if (!string.IsNullOrEmpty(str引受面積))
            {
                ...
                if (!Regex.IsMatch(str引受面積, pattern91))
                {
                    errMsg = MessageUtil.Get("ME00004", "引受面積", "8", "1", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 9. 転作等面積（任意、存在する場合、半角数字、9文字／表示形式(9,1)）
            ...
            if (!string.IsNullOrEmpty(str転作等面積))
            {
                ...
                if (!Regex.IsMatch(str転作等面積, pattern91))
                {
                    errMsg = MessageUtil.Get("ME00004", "転作等面積", "8", "1", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 10. 受委託区分（任意、存在する場合、半角数字、1桁）
            ...
            // 20. 修正日付（任意、存在する場合、日付型、10文字、形式 "yyyy/MM/dd"）
            ...
            if (!string.IsNullOrEmpty(修正日付))
            {
                ...
                {
                    errMsg = MessageUtil.Get("ME80013", "修正日付", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 21. 計算日付（任意、存在する場合、日付型、10文字、形式 "yyyy/MM/dd"）
            ...
            if (!string.IsNullOrEmpty(計算日付))
            {
                ...
                {
                    errMsg = MessageUtil.Get("ME80013", "計算日付", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }
            ...
            // 24. 統計市町村コード（任意、存在する場合、半角数字、5桁）
            string 統計市町村コード = GetField(fields, 23).Trim().Trim('"');
            ...
            // 25. 統計地域コード（任意、存在する場合、半角数字、5桁）
            string 統計地域コード = GetField(fields, 24).Trim().Trim('"');
            ...
            // 26. 統計単収（任意、存在する場合、半角数字、4文字／表示形式(zzz9)）
            string 統計単収 = GetField(fields, 25).Trim().Trim('"');
            ...
            // 27. 麦用途区分（任意、存在する場合、半角数字、3桁）
            string 麦用途区分 = GetField(fields, 26).Trim().Trim('"');
            ...
            // 28. 産地銘柄コード（任意、存在する場合、半角数字、5桁）
            string 産地銘柄コード = GetField(fields, 27).Trim().Trim('"');
            ...
            // 29. 受委託者コード（任意、存在する場合、半角数字、13桁）
            string 受委託者コード = GetField(fields, 28).Trim().Trim('"');

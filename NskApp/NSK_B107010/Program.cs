        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (NskAppContext dbContext = new(dbInfo.ConnectionString, dbInfo.DefaultSchema, ConfigUtil.GetInt(Core.CoreConst.COMMAND_TIMEOUT)))
                {
                    ...
                    // ６．帳票を作成する。
                    using (var reportController = new NskReportP107010Controller(dbInfo))
                    {
                        ControllerResult controllerResult = null;
                        // ６．１．「帳票制御処理設計書_NSK_107010P_加入承諾書兼共済掛金等払込通知書（６号）」を呼び出す。
                        controllerResult = reportController.ManageReports("NSK_107010P", jid, todofukenCd, kumiaitoCd, shishoCd, nBid);

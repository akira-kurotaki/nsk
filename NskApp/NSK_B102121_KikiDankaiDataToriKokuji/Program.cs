using NSK_B102121_KikiDankaiDataToriKokuji.Common;
...
namespace NSK_B102121_KikiDankaiDataToriKokuji
{
    /// <summary>
    /// 定時実行予約登録
    /// </summary>
            ...
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < allLines.Length; i++)
                    {
                        ...
                        else if (!IsNumeric(fields[6].Trim('"')) || fields[6].Trim('"').Length > 1)
                        {
                            acceptanceErrorContent = MessageUtil.Get("ME00020", "引受方式", "2", "(" + lineNumber + "行目)");
                            forcedShutdownFlg = true;
                            goto forcedShutdown;
                        ...
                        else if (!IsNumeric(fields[7].Trim('"')) || fields[7].Trim('"').Length > 2)
                        {
                        ...
                        else if (!IsNumeric(fields[8].Trim('"')) || fields[8].Trim('"').Length > 5)
                        {
                        ...
                        if (!string.IsNullOrWhiteSpace(fields[9].Trim('"')))
                        {
                            ...
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "共済掛金標準率", "2", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        string pattern63 = @"^\d+\.\d{1,3}$";
                        // 11. 共済掛金標準率（数値、文字数6,3）※必須ではないので、値がある場合のみチェック
                        if (!string.IsNullOrWhiteSpace(fields[10].Trim('"')))
                        {
                            
                            if (!Regex.IsMatch(fields[10].Trim('"'), pattern63) || fields[10].Trim('"').Replace(".", "").Length > 6)
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "共済掛金標準率", "6", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(fields[11].Trim('"')))
                        {
                            ...
                            if (!Regex.IsMatch(fields[11].Trim('"'), pattern63) || fields[11].Trim('"').Replace(".", "").Length > 6)
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "通常標準被害率", "6", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(fields[12].Trim('"')))
                        {
                            ...
                            if (!Regex.IsMatch(fields[12].Trim('"'), pattern63) || fields[12].Trim('"').Replace(".", "").Length > 6)
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "保険料基礎率", "6", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(fields[13].Trim('"')))
                        {
                            ...
                            if (!Regex.IsMatch(fields[13].Trim('"'), pattern63) || fields[13].Trim('"').Replace(".", "").Length > 6)
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "異常標準被害率", "6", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(fields[14].Trim('"')))
                        {
                            ...
                            if (!Regex.IsMatch(fields[14].Trim('"'), pattern63) || fields[14].Trim('"').Replace(".", "").Length > 6)
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "再保険料基礎率", "6", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        string pattern31 = @"^\d{1,2}\.\d$";
                        // 16. 国庫負担割合（数値、文字数3,1）※必須ではないので、値がある場合のみチェック
                        if (!string.IsNullOrWhiteSpace(fields[15].Trim('"')))
                        {

                            if (!Regex.IsMatch(fields[15].Trim('"'), pattern31) || fields[15].Trim('"').Replace(".", "").Length > 6)
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00020", "国庫負担割合", "6", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }

...
namespace NSK_B109040.Models
{
    ...
    class BatchJouken
    {
        ...
        /// 条件_支所コード
        /// </summary>
        public string JoukenShishoCd { get; set; } = string.Empty;
        ...
        public void GetBatchJoukens(NskAppContext dbContext, string jid)
        {
            ...
            [
                    ...
                    JoukenNameConst.JOUKEN_SHISHO,              // 支所コード
            ...
            {
                // [変数：エラーメッセージ]に、以下のエラーメッセージを設定し、「１０.」へ進む。
            ...
            foreach (T01050バッチ条件 joken in batchJokens)
            {
                switch (joken.条件名称)
                {
                    ...
                    case JoukenNameConst.JOUKEN_KYOSAI_MOKUTEKI:    // 共済目的コード　※必須
                        ...
                        break;
                    case JoukenNameConst.JOUKEN_SHISHO:             // 支所コード　※必須
                        JoukenShishoCd = joken.条件値;
                        break;
            ...
            if (string.IsNullOrEmpty(JoukenShishoCd))
            {
                // 以下のエラーメッセージを設定し、ERRORログに出力して「１０.」へ進む。
        ...
        public void IsConsistency(NskAppContext dbContext, string todofukenCd, string kumiaitoCd, string shishoCd)
        {
            ...
            if (!string.IsNullOrEmpty(JoukenShishoCd))
            {
                // 「条件_支所コード存在情報」を取得する。
                int shisho = dbContext.VShishoNms
                    .Where(x => x.TodofukenCd == todofukenCd && x.KumiaitoCd == kumiaitoCd && x.ShishoCd == JoukenShishoCd)
                    .Select(x => x.ShishoCd)
                    .Count();

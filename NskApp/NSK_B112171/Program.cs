...
namespace NSK_B112171
{
    ...
    class Program
    {
        ...
        /// バッチ名
        /// </summary>
        private static string BATCH_NAME = "加入申込書（細目・GISカンマ）大量データ受入";
            ...
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    for (int i = 3; i < allLines.Length; i++)
                    {
                        ...
                        string 局都道府県コード = (dataFields.Length > 24) ? dataFields[24].Trim().Trim('"') : "";
                        string 市区町村コード = (dataFields.Length > 25) ? dataFields[25].Trim().Trim('"') : "";
                        string 大字コード = (dataFields.Length > 26) ? dataFields[26].Trim().Trim('"') : "";
                        string 小字コード = (dataFields.Length > 27) ? dataFields[27].Trim().Trim('"') : "";
                        string 地番 = (dataFields.Length > 28) ? dataFields[28].Trim().Trim('"') : "";
                        string 枝番 = (dataFields.Length > 29) ? dataFields[29].Trim().Trim('"') : "";
                        string 子番 = (dataFields.Length > 30) ? dataFields[30].Trim().Trim('"') : "";
                        string 孫番 = (dataFields.Length > 31) ? dataFields[31].Trim().Trim('"') : "";
                        string 統計市町村コード = (dataFields.Length > 32) ? dataFields[32].Trim().Trim('"') : "";
                        string 統計地域コード = (dataFields.Length > 33) ? dataFields[33].Trim().Trim('"') : "";
                        string 統計単収 = (dataFields.Length > 34) ? dataFields[34].Trim().Trim('"') : "";
                        string 麦用途区分 = (dataFields.Length > 35) ? dataFields[35].Trim().Trim('"') : "";
                        string 産地銘柄コード = (dataFields.Length > 36) ? dataFields[36].Trim().Trim('"') : "";
                        string 受委託者コード = (dataFields.Length > 37) ? dataFields[37].Trim().Trim('"') : "";
        ...
        private static string ValidateRangeRecord(string[] fields, int lineNumber)
        {
            ...
            // ※ fields[0]～fields[8]：各項目（すでにTrim済みの場合もありますが再度Trim）
            ...
            switch (抽出区分)
            {
                case "1":
                    ...
                    // 6. 範囲パラメータ２：文字列、文字数2（入力があればチェック）
                    string field6 = fields[5].Trim();
        ...
        private static string ValidateDataRow(string[] fields, int lineNumber)
        {
            ...
            // 25. 局都道府県コード（任意、存在する場合、文字列、4文字／表示形式(9999)）
            string 局都道府県コード = GetField(fields, 24).Trim().Trim('"');
            if (!string.IsNullOrEmpty(局都道府県コード))
            {
                if (局都道府県コード.Length > 4)
                {
                    errMsg = MessageUtil.Get("ME00020", "局都道府県コード", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
                if (!IsAllDigits(局都道府県コード))
                {
                    errMsg = MessageUtil.Get("ME00003", "局都道府県コード", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 26. 市区町村コード（任意、存在する場合、文字列、3文字／表示形式(999)）
            string 市区町村コード = GetField(fields, 25).Trim().Trim('"');
            if (!string.IsNullOrEmpty(市区町村コード))
            {
                if (市区町村コード.Length > 3)
                {
                    errMsg = MessageUtil.Get("ME00020", "市区町村コード", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
                if (!IsAllDigits(市区町村コード))
                {
                    errMsg = MessageUtil.Get("ME00003", "市区町村コード", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 27. 大字コード（任意、存在する場合、文字列、8文字／表示形式(99999999)）
            string 大字コード = GetField(fields, 26).Trim().Trim('"');
            if (!string.IsNullOrEmpty(大字コード))
            {
                if (大字コード.Length > 8)
                {
                    errMsg = MessageUtil.Get("ME00020", "大字コード", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
                if (!IsAllDigits(大字コード))
                {
                    errMsg = MessageUtil.Get("ME00003", "大字コード", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 28. 小字コード（任意、存在する場合、文字列、4文字／表示形式(9999)）
            string 小字コード = GetField(fields, 27).Trim().Trim('"');
            if (!string.IsNullOrEmpty(小字コード))
            {
                if (小字コード.Length > 4)
                {
                    errMsg = MessageUtil.Get("ME00020", "小字コード", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
                if (!IsAllDigits(小字コード))
                {
                    errMsg = MessageUtil.Get("ME00003", "小字コード", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 29. 地番（任意、存在する場合、文字列、16文字）
            string 地番 = GetField(fields, 28).Trim().Trim('"');
            if (!string.IsNullOrEmpty(地番))
            {
                if (地番.Length > 16)
                {
                    errMsg = MessageUtil.Get("ME00020", "地番", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 30. 枝番（任意、存在する場合、文字列、14文字）
            string 枝番 = GetField(fields, 29).Trim().Trim('"');
            if (!string.IsNullOrEmpty(枝番))
            {
                if (枝番.Length > 14)
                {
                    errMsg = MessageUtil.Get("ME00020", "枝番", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 31. 子番（任意、存在する場合、文字列、10文字）
            string 子番 = GetField(fields, 30).Trim().Trim('"');
            if (!string.IsNullOrEmpty(子番))
            {
                if (子番.Length > 10)
                {
                    errMsg = MessageUtil.Get("ME00020", "子番", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 32. 孫番（任意、存在する場合、文字列、10文字）
            string 孫番 = GetField(fields, 31).Trim().Trim('"');
            if (!string.IsNullOrEmpty(孫番))
            {
                if (孫番.Length > 10)
                {
                    errMsg = MessageUtil.Get("ME00020", "孫番", "桁数", "(" + lineNumber + "行目)");
                    logger.Error(errMsg);
                    return errMsg;
                }
            }

            // 33. 統計市町村コード（任意、存在する場合、半角数字、5桁）
            string 統計市町村コード = GetField(fields, 32).Trim().Trim('"');
            ...
            // 34. 統計地域コード（任意、存在する場合、半角数字、5桁）
            string 統計地域コード = GetField(fields, 33).Trim().Trim('"');
            ...
            // 35. 統計単収（任意、存在する場合、半角数字、4文字／表示形式(zzz9)）
            string 統計単収 = GetField(fields, 34).Trim().Trim('"');
            ...
            // 36. 麦用途区分（任意、存在する場合、半角数字、3桁）
            string 麦用途区分 = GetField(fields, 35).Trim().Trim('"');
            ...
            // 37. 産地銘柄コード（任意、存在する場合、半角数字、5桁）
            string 産地銘柄コード = GetField(fields, 36).Trim().Trim('"');
            ...
            // 38. 受委託者コード（任意、存在する場合、半角数字、13桁）
            string 受委託者コード = GetField(fields, 37).Trim().Trim('"');

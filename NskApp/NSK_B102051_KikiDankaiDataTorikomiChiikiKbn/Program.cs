using NSK_B102051_KikiDankaiDataTorikomiChiikiKbn.Common;
...
namespace NSK_B102051_KikiDankaiDataTorikomiChiikiKbn
{
    /// <summary>
    /// 定時実行予約登録
    /// </summary>
            ...
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < allLines.Length; i++)
                    {
                        ...
                        else if (!IsNumeric(fields[4]) || fields[4].Length > 4)
                        {
                        ...
                        else if (!IsNumeric(fields[6]) || fields[6].Length > 3)
                        {
                        ...
                        string 類区分 = fields[3];
                        string 大地区コード = fields[4];
                        string 小地区コード = fields[5];
                        string 引受方式 = fields[6];
                        string 特約区分 = fields[7];
                        string 補償割合コード = fields[8];
                        string 危険段階地域区分 = fields[9];

...
namespace NSK_B105160.Models
{
    ...
    public class BatchJoken
    {
        ...
        /// 営農対象外フラグ
        /// </summary>
        public string JoukenEinoTaishogaiFlag { get; set; } = string.Empty;
        ...
        /// <returns></returns>
        public static List<KijunShukakuryoSetteiRecord> GetKijunShukakuryoSettei(NskAppContext dbContext, string todofukenCd, string kumiaitoCd, string shishoCd, BatchJoken batchJoken)
        {

            // ７．１．[配列：バッチ条件]の共済目的の値をもとにSQLに使用する [変数：引受方式] を設定する。
            // ※共済目的：陸稲の災害収入、品質はない想定。
            // 　万が一陸稲のデータが存在した場合は初期値の0のままとし、出力時にnullを表示する。
            int hikiukeHoshiki = 0;
            if (batchJoken.JokenKyosaiMokutekiCd == $"{(int)KyosaiMokutekiCdNumber.Suitou}")
            {
                // 引受方式に5：水稲品質を設定
                hikiukeHoshiki = (int)HikiukeHoushikiCd.Hinshitsu;
            }
            else if (batchJoken.JokenKyosaiMokutekiCd == $"{(int)KyosaiMokutekiCdNumber.Mugi}")
            {
                // 引受方式に4：災害収入を設定
                hikiukeHoshiki = (int)HikiukeHoushikiCd.SaigaiSyuunyuu;
            }

            // ７．３．対象データの件数を取得
            // ７．３．１．「共済金額一覧表」の件数を取得する。
            // ７．４．対象データを取得
            // 件数のみの取得はせず、対象データを取得して件数の判定を行うため、７．２．はスキップ
            // ７．４．→７．３．の処理順となる。

            StringBuilder sql = new();

            // 産地別銘柄別引受情報の規格数
            int kikakuMaxCount = 40;
            ...
            // 共済目的が陸稲以外の場合はm_10080_引受方式名称から取得した引受方式を出力し、
            // 陸稲の場合はnullを出力
            if (!(batchJoken.JokenKyosaiMokutekiCd == $"{(int)KyosaiMokutekiCdNumber.Rikutou}"))
            {
                sql.Append($"  ,T10.引受方式 ");
                sql.Append($"  ,T10.引受方式名称 ");
            }
            else
            {
                sql.Append("  ,null AS 引受方式 ");
                sql.Append("  ,null AS 引受方式名称 ");
            }
            ...
            sql.Append($"  ,T9.産地別銘柄名称 ");
            sql.Append($"  ,T1.営農対象外フラグ ");
            sql.Append($"  ,T1.平均単収 ");
            
            // 規格別割合の動的取得
            for (int x = 1; x <= kikakuMaxCount; x++)
            {
                sql.Append($"  ,T12_" + x + ".規格別割合 AS 規格別割合_規格" + x);
            }
            #endregion

            #region 検索テーブル
            sql.Append($" FROM ");
            sql.Append($"   t_11060_基準収穫量設定 T1 ");
            ...
            sql.Append($"   INNER JOIN  m_00130_産地別銘柄名称設定 T9 ");
            sql.Append($"     ON T1.組合等コード = T9.組合等コード ");
            sql.Append($"     AND T1.共済目的コード = T9.共済目的コード ");
            sql.Append($"     AND T1.産地別銘柄コード = T9.産地別銘柄コード ");
            sql.Append($"     AND T1.年産 = T9.年産 ");
            sql.Append($"   LEFT OUTER JOIN m_10080_引受方式名称 T10 ");
            sql.Append($"     ON T10.引受方式 = @引受方式 ");
            sql.Append($"   INNER JOIN  m_00230_営農対象名称 T11 ");
            sql.Append($"     ON T1.営農対象外フラグ = T11.営農対象外フラグ ");

            // t_11070_基準収穫量設定_規格別の動的結合
            for (int x = 1; x <= kikakuMaxCount; x++)
            {
                sql.Append($"   LEFT OUTER JOIN t_11070_基準収穫量設定_規格別 T12_" + x + " ");
                sql.Append($"     ON T1.組合等コード = T12_" + x + ".組合等コード ");
                sql.Append($"     AND T1.年産 = T12_" + x + ".年産 ");
                sql.Append($"     AND T1.共済目的コード = T12_" + x + ".共済目的コード ");
                sql.Append($"     AND T1.組合員等コード = T12_" + x + ".組合員等コード ");
                sql.Append($"     AND T1.類区分 = T12_" + x + ".類区分 ");
                sql.Append($"     AND T1.産地別銘柄コード = T12_" + x + ".産地別銘柄コード ");
                sql.Append($"     AND T1.営農対象外フラグ = T12_" + x + ".営農対象外フラグ ");
                sql.Append($"     AND T12_" + x + ".規格 = '" + x + "' ");
            ...
            [
                ...
                new("引受方式", hikiukeHoshiki.ToString()),
            ];
            ...
            // 類区分の指定がある場合
            if (!string.IsNullOrWhiteSpace(batchJoken.JoukenEinoTaishogaiFlag))
            {
                sql.Append($"   AND T1.営農対象外フラグ = @条件_営農対象外フラグ ");
                parameters.Add(new("条件_営農対象外フラグ", batchJoken.JoukenEinoTaishogaiFlag));
            }
            ...
            bool einoTaishogaiFlagSort = false;
                ...
                if (!string.IsNullOrWhiteSpace(batchJoken.JokenOrderByKey1))
                {
                    ...
                    switch (batchJoken.JokenOrderByKey1)
                    {
                        ...
                        case Constants.ruikbnSortName:
                            ...
                            ruikbnSort = true;
                            break;
                        case Constants.einoTaishogaiFlagSortName:
                            sql.Append($" T1.営農対象外フラグ {batchJoken.JokenOrderBy1} ");
                            einoTaishogaiFlagSort = true;
                            break;
                ...
                if (!string.IsNullOrWhiteSpace(batchJoken.JokenOrderByKey2))
                {
                    ...
                    switch (batchJoken.JokenOrderByKey2)
                    {
                        ...
                        case Constants.ruikbnSortName:
                            ...
                            break;
                        case Constants.einoTaishogaiFlagSortName:
                            sql.Append($" T1.営農対象外フラグ {batchJoken.JokenOrderBy2} ");
                            einoTaishogaiFlagSort = true;
                            break;
                        case Constants.updateDateSortName:
                ...
                if (!string.IsNullOrWhiteSpace(batchJoken.JokenOrderByKey3))
                {
                    ...
                    switch (batchJoken.JokenOrderByKey3)
                    {
                        ...
                        case Constants.ruikbnSortName:
                            ...
                            break;
                        case Constants.einoTaishogaiFlagSortName:
                            sql.Append($" T1.営農対象外フラグ {batchJoken.JokenOrderBy3} ");
                            einoTaishogaiFlagSort = true;
                            break;
                        case Constants.updateDateSortName:
            ...
            sql.Append(" ,T1.産地別銘柄コード Asc ");
            // 「表示順キー」で営農対象外フラグの指定がない場合
            if (!einoTaishogaiFlagSort)
                sql.Append(" ,T1.営農対象外フラグ Asc ");
            // 「表示順キー」で更新日時の指定がない場合
            if (!updateDateSort)
            ...
            // SQLのクエリ結果をListに格納する
            List<KijunShukakuryoSetteiRecord> KijunShukakuryoSettei = dbContext.Database.SqlQueryRaw<KijunShukakuryoSetteiRecord>(sql.ToString(), parameters.ToArray()).ToList();

            return KijunShukakuryoSettei;
        }
        ...
        public void GetBatchJoken(NskAppContext dbContext, string jid)
        {
            ...
            [
                ...
                JoukenNameConst.JOUKEN_EINO_TAISHOGAI_FLG,        //営農対象外フラグ
            ...
            foreach (T01050バッチ条件 joken in batchJokens)
            {
                switch (joken.条件名称)
                {
                    ...
                    case JoukenNameConst.JOUKEN_RUI_KBN:               // 類区分
                        ...
                        break;
                    case JoukenNameConst.JOUKEN_EINO_TAISHOGAI_FLG:    // 営農対象外フラグ
                        JoukenEinoTaishogaiFlag = joken.条件値;
                        break;

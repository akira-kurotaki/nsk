...
namespace NSK_B111041
{
    /// <summary>
    /// 交付金申請書データ作成連合会交付金申請明細部
    /// </summary>
    class Program
    {
        /// <summary>
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (NskAppContext dbContext = new(dbInfo.ConnectionString, dbInfo.DefaultSchema, ConfigUtil.GetInt("CommandTimeout")))
                {
                    ...
                    // ８．連合会交付金申請明細部データ出力処理
                    // ８．１．[変数：条件_文字コード]で指定した文字コードの連合会交付金申請明細部データファイル作成
                    ...
                    // ファイル名："350" & [変数：条件_負担金交付区分] & "01" & [変数：条件_組合等コード] & ".TXT"
                    string fileName = "350X01YY".Replace("X", batchJoken.JokenFutankinKofuKbn).Replace("YY", batchJoken.JokenKumiaitoCd) + NskCommon.CoreConst.FILE_EXTENSION_TXT;
                    string filePath = Path.Combine(tempFolderPath, fileName);

                    // ８．２．取得した連合会交付金申請明細部データを連合会交付金申請明細部データファイルにカンマ区切りで出力
                    // ファイル設計書に沿って連合会交付金申請明細部ファイルに連合会交付金申請明細部データを出力する。
                        ...
                        using (StreamWriter writer = new(fs, encoding))
                        {
                            ...
                            foreach (KoufuKinShinseiRecord koufuKinShinseiRecord in koufuKinShinsei)
                            {
                                ...
                                [
                                    ...
                                    todofukenCd,
                                    // 組合等番号
                                    koufuKinShinseiRecord.組合等コード,
                                    // 合併特例有無
                                    "0",
                                    // 組合等合併時識別番号
                                    koufuKinShinseiRecord.組合等合併時識別番号,
                                    // 交付回
                                    koufuKinShinseiRecord.交付回.ToString(),
                                    // 組合等別国庫負担金
                                    koufuKinShinseiRecord.組合等別国庫負担金.ToString(),
                                    // 組合等別連合会保険料
                                    koufuKinShinseiRecord.組合等別連合会保険料.ToString(),
                                    // 組合等別再保険料
                                    "",
                                    // 組合等別連合会負担金交付対象額
                                    koufuKinShinseiRecord.組合等別連合会負担金交付対象額.ToString(),
                                    // 組合員等負担共済掛金
                                    koufuKinShinseiRecord.組合員等負担共済掛金.ToString(),
                                    // 組合員等負担共済掛金徴収済額
                                    koufuKinShinseiRecord.組合員等負担共済掛金徴収済額.ToString(),
                                    // 共済掛金徴収割合
                                    koufuKinShinseiRecord.共済掛金徴収割合.ToString(),
                                    // 組合等交付金の金額

            ...
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < allLines.Length; i++)
                    {
                        ...
                        else if (fields[3].Trim('"').Length > 3)
                        {
                        ...
                        {
                            acceptanceErrorContent = MessageUtil.Get("ME00003", "地域単位区分", "(" + lineNumber + "行目)");
                            forcedShutdownFlg = true;
                            goto forcedShutdown;
                        }
                        else if (fields[5].Trim('"').Length > 5)
                        {
                        ...
                        {
                            acceptanceErrorContent = MessageUtil.Get("ME00003", "危険段階区分", "(" + lineNumber + "行目)");
                            forcedShutdownFlg = true;
                            goto forcedShutdown;
                        }
                        ...
                        // 11. 危険段階基準共済掛金率（数値、文字数6）※必須ではないので、値がある場合のみチェック
                        if (!string.IsNullOrWhiteSpace(fields[10].Trim('"')))
                        {
                            if (!Regex.IsMatch(fields[10].Trim('"'), pattern))
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00004", "危険段階基準共済掛金率", "3", "3", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }

                        // 12. 危険段階共済掛金率（数値、文字数6）※必須ではないので、値がある場合のみチェック
                        if (!string.IsNullOrWhiteSpace(fields[11].Trim('"')))
                        {
                            if (!Regex.IsMatch(fields[11].Trim('"'), pattern))
                            {
                                acceptanceErrorContent = MessageUtil.Get("ME00004", "危険段階共済掛金率", "3", "3", "(" + lineNumber + "行目)");
                                forcedShutdownFlg = true;
                                goto forcedShutdown;
                            }
                        }
                        ...
                        string 地域単位区分 = fields[5].Trim('"');
                        string 引受方式 = fields[6].Trim('"');
                        string 特約区分 = fields[7].Trim('"');
                        string 補償割合コード = fields[8].Trim('"');
                        string 危険段階区分 = fields[9].Trim('"');
                        string 危険段階基準共済掛金率 = fields[10].Trim('"');
                        string 危険段階共済掛金率 = fields[11].Trim('"');
                ...
                finally
                {
                    ...
                    //Directory.Delete((new DirectoryInfo(tempFilePath)).Parent.FullName, true);

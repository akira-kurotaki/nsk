        ...
        static void Main(string[] args)
        {
            ...
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    // バッチ条件情報取得
                    List<T01050バッチ条件> joukenList = GetBatchJoukenList(db, jid, Constants.JOUKEN_NENSAN, Constants.JOUKEN_FILE_PATH, Constants.JOUKEN_UKEIRERIREKI_ID, Constants.JOUKEN_FILE_HASH);
...
　
                    ...
                    skipByError:;
                        ...
                        else
                        {
                            var entity = new T19050大量データ受入組合員等類別設定ok
                            {
                                ...
                                共済金額選択順位 = 共済金額選択順位F,
                                収穫量確認方法 = 収穫量確認方法F,
                                全相殺基準単収 = string.IsNullOrEmpty(全相殺基準単収F) ? 0 : decimal.Parse(全相殺基準単収F),
                ...
                forcedShutdown:;
                    ...
                    if (1 <= errCount)
                    {
                        ...
                        string errFileName = $"{preNm}-ERR-{ukeirerirekiId}.csv";

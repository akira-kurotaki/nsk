using NSK_B112091_RuibetuHeikinTanshuDataUkeire.Common;
...
namespace NSK_B112091_RuibetuHeikinTanshuDataUkeire
{
    /// <summary>
    /// 定時実行予約登録
    /// </summary>
    class Program
    {
        ...
        private const string DestinationFolder = @"C:\NSK112051B\ErrorFiles";
            ...
            using (var db = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema, ConfigUtil.GetInt(Constants.CONFIG_COMMAND_TIMEOUT)))
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < allLines.Length; i++)
                    {
                        ...
                        string 組合等コードF = fields[0];
                        string 年産F = fields[1];
                        string 共済目的コード = fields[2];
                        string 組合員等コード = fields[3];
                        string 類区分 = fields[4];
                        string 引受区分 = fields[5];
                        string 全相殺基準単収 = fields[6];

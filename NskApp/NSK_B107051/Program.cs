...
using NSK_B107051.Models;
using ModelLibrary.Context;
using NpgsqlTypes;

namespace NSK_B107051
{
    /// <summary>
    /// 消込み処理（還付・自動）（インタフェース）
    /// </summary>
    class Program
    {
        ...
        private static List<SubSaimokuDataRecord> GetSubSaimokuData(NskAppContext dbContext, string kumiaitoCd, BatchJoken batchJoken)
        {
            ...
            sql.Append($@"WHERE ");
            sql.Append($@"    1 = 1 ");
            sql.Append($@"    AND 組合等コード = @組合等コード ");
            sql.Append($@"    AND 年産 = @バッチ条件年産 ");
            sql.Append($@"    AND 共済目的コード = @バッチ条件共済目的コード ");
            sql.Append($@"    AND 共済事業コード = '01' ");
            sql.Append($@"    AND 金額区分 = '2' ");

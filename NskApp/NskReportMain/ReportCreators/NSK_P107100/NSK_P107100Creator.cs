using NskReportMain.Models.NSK_P107100;
...
namespace NskReportMain.ReportCreators.NSK_P107100
{
    /// <summary>
    /// P107100_未納者リスト
    /// </summary>
    public class NSK_P107100Creator : ReportCreator
    {
            ...
            List<NSK_P107100Model> model)
        {
            ...
            // 帳票を作成する
            report = new BaseSectionReport(@"Reports\P107100\P107100Report.rpx");
            // 帳票のデータ設定
            ...
            ref SectionReport rpt)
        {
            ...
            bool genYotei = false;
            bool chuiGaki = false;
            var jouken = reportJoukens.Where(t => t.条件名称 == JoukenNameConst.JOUKEN_GENKIN_CHOSHU).SingleOrDefault();
            if (!string.IsNullOrEmpty(jouken?.条件値))
            {
                genChoshuFlg = jouken.条件値;
                genYotei = JOUKEN_GENKIN_CHOSHU_ARI.Equals(genChoshuFlg);
                chuiGaki = JOUKEN_GENKIN_CHOSHU_NASHI.Equals(genChoshuFlg);
            }
            ...
            ((Label)rpt.Sections["GroupHeader1"].Controls["lblBikoCashOnly"]).Visible = genYotei;
            ((Label)rpt.Sections["GroupHeader1"].Controls["lblBiko1"]).Visible = chuiGaki;
            ((Label)rpt.Sections["GroupHeader1"].Controls["lblBiko2"]).Visible = chuiGaki;
            ...
            for (int i = 0; i < model.Count; i++)
            {
                ...
                // 金融機関コード
                item.金融機関コード = item.金融機関コード is null ? string.Empty : item.金融機関コード.PadLeft(7, '0');

                // 口座番号
                item.口座番号 = item.口座番号 is null ? string.Empty : item.口座番号.PadLeft(10, '0');

                // 特別防災賦課金
                item.特別防災賦課金 = item.特別賦課金 + item.防災賦課金;

                // 徴収済額
                item.徴収済額 = item.今回迄徴収額 - item.今回迄引受解除徴収賦課金額;

                // 未納額
                item.未納額 = (item.納入額 - (item.今回迄徴収額 - item.今回迄引受解除徴収賦課金額)) > 0 ? (item.納入額 - (item.今回迄徴収額 - item.今回迄引受解除徴収賦課金額)) : 0;

                // 還付額
                item.還付額 = ((item.今回迄徴収額 - item.今回迄引受解除徴収賦課金額) - item.納入額) > 0 ? ((item.今回迄徴収額 - item.今回迄引受解除徴収賦課金額) - item.納入額) : 0;

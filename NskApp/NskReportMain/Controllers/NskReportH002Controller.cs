...
using NskReportMain.Models.H002;
using NskReportMain.Models.P1002;
using NskReportMain.ReportCreators.P1002;
...
{
    public class NskReportH002Controller : ReportController
    {
        ...
        public NskReportH002Controller(DbConnectionInfo dbInfo) : base(dbInfo)
        {
        }
        ...
        public ControllerResult ManageReports(string userId, int joukenId, string todofukenCd, string kumiaitoCd, string shishoCd, List<string> shishoList, int batchFlag, string filePath, long? batchId)
        {
            ...
            try
            {
                ...
                var reportJoukens = GetReportJouken(getJigyoDb<NskAppContext>(), joukenId);
            ...
            List<string> shishoList, int batchFlag, string filePath)
        {
            ...
            if (ReportConst.REPORT_REALTIME.Equals(batchFlag))
            {
                ...
                if (!shishoList.IsNullOrEmpty())
                {
                    ...
                    var shishoCnt = getJigyoDb<NskAppContext>().VShishoNms.Where(t => t.TodofukenCd == todofukenCd && t.KumiaitoCd == kumiaitoCd && shishoList.Contains(t.ShishoCd)).Count();
                    if (shishoCnt == 0)
                    {
        ...
        private List<P1002Model> GetRealReportDataList(List<TReportJouken> reportJoukens, string todofukenCd, string kumiaitoCd, string shishoCd, List<string> shishoList, int batchFlag)
        {
            ...
            return getJigyoDb<NskAppContext>().Database.SqlQueryRaw<P1002Model>(sql.ToString(), parameters.ToArray()).ToList();
        }
        #endregion
        ...
        private List<H002GroupInfoModel> GetGroupInfoList(List<TReportJouken> reportJoukens, string todofukenCd, string kumiaitoCd, string shishoCd, List<string> shishoList, int batchFlag)
        {
            ...
            return getJigyoDb<NskAppContext>().Database.SqlQueryRaw<H002GroupInfoModel>(sql.ToString(), parameters.ToArray()).ToList();
        }
        #endregion
        ...
        private List<int> GetNogyoshaIdList(List<TReportJouken> reportJoukens, H002GroupInfoModel groupInfo)
        {
            ...
            return getJigyoDb<NskAppContext>().Database.SqlQueryRaw<int>(sql.ToString(), parameters.ToArray()).ToList();
        }
        #endregion

...
using NSK_B105012.Models;
using NSK_B105012.Common;
using NskCommonLibrary.Core.Consts;

namespace NSK_B105012
{
    /// <summary>
    /// 細目データ出力処理GISカンマ編集データ
    /// </summary>
    class Program
    {
        /// <summary>
                        ...
                        using (StreamWriter writer = new(fs, encoding))
                        {
                            ...
                            List<string> haniRecords =
                                [
                                    ...
                                    Constants.GIS_DATA_OUTPUT_TYPE_COMMA_EDIT
                                ];
                            // 配列の内容を書き込む
                            ...
                            foreach (SaimokuDataRecord saimokuRecord in saimokuDataRecs)
                            {
                                ...
                                [
                                    ...
                                    saimokuRecord.RS区分,
                                    saimokuRecord.局都道府県コード,
                                    saimokuRecord.市区町村コード,
                                    saimokuRecord.大字コード,
                                    saimokuRecord.小字コード,
                                    saimokuRecord.地番,
                                    saimokuRecord.枝番,
        ...
        private static List<SaimokuDataRecord> GetSaimokuData(NskAppContext dbContext, string todofukenCd, string kumiaitoCd, BatchJoken batchJoken)
        {
            ...
            sql.Append($@"    T4.rs区分, ");
            sql.Append($@"    T4.局都道府県コード, ");
            sql.Append($@"    T4.市区町村コード, ");
            sql.Append($@"    T4.大字コード, ");
            sql.Append($@"    T4.小字コード, ");
            sql.Append($@"    T4.地番, ");
            sql.Append($@"    T4.枝番, ");

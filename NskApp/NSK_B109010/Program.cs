...
using NSK_P109010.Controllers;
using System.Text;

namespace NSK_B109010
{
    /// <summary>
    /// 加入申込書兼変更届出書
    /// </summary>
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                try
                {
                    ...
                    {
                        // 以下のエラーメッセージを[変数：エラーメッセージ] に設定し、「７.」へ進む。
                        // （"ME01054" 引数{0} ：バッチID）
                    ...
                    using (NskAppContext dbContext = new(dbInfo.ConnectionString, dbInfo.DefaultSchema, ConfigUtil.GetInt(Core.CoreConst.COMMAND_TIMEOUT)))
                    {
                        ...
                        // ６．帳票を作成する。
                        // ６．１．「帳票制御処理_NSK_109010P_加入申込書兼変更届出書」を呼び出す。
                        // 【引数】
                        List<string> hikisu =
                        [
                            "NSK_109010P",
                        ...
                        using (NSK_109010PController nsk109010PController = new(dbInfo))
                        {
                            controllerResult = nsk109010PController.ManageReports(dbContext, hikisu.ToArray());
                        }

                        // ６．１．２．失敗した(戻り値(0)以外）場合
                        if (controllerResult.Result != 0)
                        {
                            // 戻り値で返されたエラーメッセージを、[変数：エラーメッセージ]に設定し、「７．」へ進む。

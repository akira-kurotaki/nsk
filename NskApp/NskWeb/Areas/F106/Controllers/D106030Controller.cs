...
using NskWeb.Areas.F106.Models.D106030;
...
namespace NskWeb.Areas.F106.Controllers
{
    ...
    [SessionOutCheck]
    [Area("F106")]
    public class D106030Controller : D106000BaseController
    {
        #region メンバー定数
        /// <summary>
        /// 画面ID(D106030)
        /// </summary>
        private static readonly string SCREEN_ID_D106030 = "D106030";

        /// <summary>
        /// セッションキー(D106030)
        /// </summary>
        private static readonly string SESS_D106030 = SCREEN_ID_D106030 + "_" + "SCREEN";

        /// <summary>
        /// 引受回一覧ビュー名（検索結果）
        /// </summary>
        private static readonly string RESULT_VIEW_NAME = "_D106030SearchResult";

        /// <summary>
        /// 画面名
        /// </summary>
        private static readonly string D106030_SCREEN_NM = "NSK_106030D";

        /// <summary>
        /// バッチ名
        /// </summary>
        private static readonly string D106030_BATCH_NM = "NSK_106031B";
        
        /// <summary>
        /// 複数実行禁止ID
        /// </summary>
        private static readonly string D106030_MULTI_ID = "NSK_106030B";

        /// <summary>
        /// 実行単位区分本所本所
        /// </summary>
        private static readonly string D106030_HIKIUKE_JIKKO_TANNI_KBN_HONSHO = "1";
        
        /// <summary>
        /// 実行単位区分本所支所
        /// </summary>
        private static readonly string D106030_HIKIUKE_JIKKO_TANNI_KBN_HONSYOSHISHO = "2";
        
        /// <summary>
        /// 実行単位区分支所支所
        /// </summary>
        private static readonly string D106030_HIKIUKE_JIKKO_TANNI_KBN_SHISHO = "3";
        #endregion
        ...
        public D106030Controller(ICompositeViewEngine viewEngine, ReportServiceClient serviceClient) : base(viewEngine, serviceClient)
        {
        }
        #endregion
        ...
        public ActionResult Init()
        {
            ...
            SessionUtil.Remove(SESS_D106030, HttpContext);

            // ログインユーザの参照・更新可否判定
            // 画面IDをキーとして、画面マスタ、画面機能権限マスタを参照し、ログインユーザに本画面の権限がない場合は業務エラー画面を表示する。
            if (!ScreenSosaUtil.CanReference(SCREEN_ID_D106030, HttpContext))
            {
            ...
            // 画面モデルの生成
            D106030Model model = new D106030Model(Syokuin, shishoList);

            if (!ScreenSosaUtil.CanUpdate(SCREEN_ID_D106030, HttpContext))
            {
            ...
            // 検索条件をセッションに保存する
            SessionUtil.Set(SESS_D106030, model, HttpContext);
            ...
            // 本所・支所ドロップダウンリストの取得
            if (ShishoJikkoHikiukeKbn.Equals(D106030_HIKIUKE_JIKKO_TANNI_KBN_HONSHO))
            {
            ...
            // 引受計算処理（麦）画面を表示する
            return View(SCREEN_ID_D106030, GetInitUnderwritingCycle(model, m, selectvalue));
        }
        #endregion
        ...
        protected List<SelectListItem> setHonshoList(D106030Model model)
        {
        ...
        /// <param name="model">画面viewモデル</param>
        /// <returns>List<SelectListItem></returns>
        protected List<SelectListItem> setShishoList(D106030Model model)
        {
        ...
        private D106030Model GetInitUnderwritingCycle(D106030Model model, NSKPortalInfoModel portalmodel, string selectvalue)
        {
            // モデル状態ディクショナリからすべての項目を削除します。
            ModelState.Clear();
            // 引受回テーブルをクリアする
            model.SearchResult = new D106030SearchResult();
        ...
        /// <returns>引受回テーブルモデル</returns>
        [HttpPost]
        public ActionResult DispUnderwritingCycle(D106030Model form)
        {
            // セッションから画面モデルを取得する
            var model = SessionUtil.Get<D106030Model>(SESS_D106030, HttpContext);
            ...
            ModelState.Clear();
            // 引受回テーブルをクリアする
            form.SearchResult = new D106030SearchResult();
        ...
        private List<D106030TableRecord> GetResultList(StringBuilder Sql, List<NpgsqlParameter> parameters)
        {
            logger.Info("引受回数表示結果情報取得処理を実行します。");

            // 引受回表示検索結果データを取得
            var sqlResults = getJigyoDb<NskAppContext>().Database.SqlQueryRaw<D106030TableRecord>(Sql.ToString(), parameters.ToArray()).ToList(); ;
        ...
        /// 実行ボタン制御値取得メソッド
        /// <summary>
        /// <param name="TableRecords">List<D106030TableRecord></param>
        /// <returns>実行ボタン制御値</returns>
        private string GetEnterCntrlFlag(List<D106030TableRecord> TableRecords)
        {
            string result = "1";
        ...
        public ActionResult CreatBatchHikiukeCalc(D106030Model model)
        {
            // システム日時
            ...
            // 複数実行禁止IDを設定する
            String multiNotId = D106030_MULTI_ID;
            ...
            // メッセージ用の支所名称を設定
            string msgShisho = model.SearchCondition.SelectShishoNm + " の引受計算処理(麦)のバッチ登録";
            // レスポンスメッセージ用変数定義
            ...
            try
            {
                ...
                using (var tx = jigyodb.Database.BeginTransaction())
                {
                    try
                    {
                        logger.Info(string.Format("引受計算（麦）のバッチ条件を登録します。"));
                        //引受年産を条件テーブルに登録
            ...
            {
                // （出力メッセージ：バッチ予約登録失敗）
                // （出力メッセージ：（メッセージID：ME01645、引数{0}："本所・支所コード+ " " + 本所・支所名 + の引受計算処理(麦)のバッチ登録"））
            ...
            {
                // （出力メッセージ：バッチ予約登録失敗）
                // （出力メッセージ：（メッセージID：ME90008、引数{0}："本所・支所コード+ " " + 本所・支所名 + の引受計算処理(麦)のバッチ登録）"））
                logger.Error("バッチ予約登録失敗");
                logger.Error(MessageUtil.Get("ME01645", msgShisho));

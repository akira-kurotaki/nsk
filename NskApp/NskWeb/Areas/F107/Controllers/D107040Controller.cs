...
namespace NskWeb.Areas.F107.Controllers
{
    ...
    [SessionOutCheck]
    [Area("F107")]
    public class D107040Controller : D107000BaseController
    {
        #region メンバー定数
        /// <summary>
        /// 画面ID(D107040)
        /// </summary>
        private static readonly string SCREEN_ID_D107040 = "D107040";

        /// <summary>
        /// セッションキー(D107040)
        /// </summary>
        private static readonly string SESS_D107040 = SCREEN_ID_D107040 + "_" + "SCREEN";

        /// <summary>
        /// バッチ名
        /// </summary>
        private static readonly string D107040_BATCH_NM = "NSK_107041B";

        /// <summary>
        /// 複数実行禁止ID
        /// </summary>
        private static readonly string D107040_MULTI_ID = "NSK_107040D";

        /// <summary>
        /// 実行単位区分本所本所
        /// </summary>
        private static readonly string D107040_HIKIUKE_JIKKO_TANNI_KBN_HONSHO = "1";
        
        /// <summary>
        /// 実行単位区分本所支所
        /// </summary>
        private static readonly string D107040_HIKIUKE_JIKKO_TANNI_KBN_HONSYOSHISHO = "2";
        
        /// <summary>
        /// 実行単位区分支所支所
        /// </summary>
        private static readonly string D107040_HIKIUKE_JIKKO_TANNI_KBN_SHISHO = "3";
        #endregion
        ...
        public D107040Controller(ICompositeViewEngine viewEngine, ReportServiceClient serviceClient) : base(viewEngine, serviceClient)
        {
        }
        #endregion
        ...
        public ActionResult Init()
        {
            ...
            if (!ScreenSosaUtil.CanReference(SCREEN_ID_D107040, HttpContext) && !ScreenSosaUtil.CanUpdate(SCREEN_ID_D107040, HttpContext))
            {
            ...
            SessionUtil.Remove(SESS_D107040, HttpContext);

            // 利用可能な支所一覧
            var shishoList = SessionUtil.Get<List<Shisho>>(CoreConst.SESS_SHISHO_GROUP, HttpContext);

            // 画面項目の初期化
            model = new D107040Model(Syokuin, shishoList);

            // 検索条件をセッションに保存する
            SessionUtil.Set(SESS_D107040, model, HttpContext);
            ...
            // 消込み処理（自動）（インタフェース）画面を表示する
            return View(SCREEN_ID_D107040, model);
        }
        #endregion
        ...
        protected List<SelectListItem> setHonshoShishoList(D107040Model model, NSKPortalInfoModel pmodel)
        {
        ...
        public ActionResult DispUnderwritingCycle(D107040Model form)
        {
            // sql用定数定義
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Insert(D107040Model form)
        {
            // セッションから画面モデルを取得する
            var model = SessionUtil.Get<D107040Model>(SESS_D107040, HttpContext);
            ...
            {
                // バッチ予約状況を取得
                var BatchYoyakuparam = BatchYoyakuJokyoParam(syokuin, ShishoCd, D107040_BATCH_NM);
            ...
            {
                // （出力メッセージ：バッチ予約状況取得失敗）
                // （出力メッセージ：（メッセージID：ME10019、引数{0}："消込み処理(自動)(インタフェース)"））
                logger.Error("バッチ予約状況取得失敗");
                responseMsg = MessageUtil.Get("ME10019", "消込み処理(自動)(インタフェース)");
            ...
            try {
                ...
                using (var tx = jigyodb.Database.BeginTransaction())
                {
                    try
                    {
                        logger.Info(string.Format("消込み処理(自動)(インタフェース)のバッチ条件を登録します。"));
                        // 先に条件テーブルへの登録を実行
                        // 引受年産を条件テーブルに登録
            ...
            // 画面情報をセッションに保存する
            SessionUtil.Set(SESS_D107040, form, HttpContext);

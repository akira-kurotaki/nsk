...
using NskWeb.Areas.F103.Models.D103010;
...
using NskWeb.Areas.F103.Consts;

namespace NskWeb.Areas.F103.Controllers
{
    /// <summary>
    /// NSK_102010D_基準統計単収取込処理（テキスト）
    /// </summary>
    /// <remarks>
    /// 作成日：2025/01/08
    /// 作成者：ジョンジュンヒョ
    ...
    [AllowAnonymous]
    [Area("F103")]
    public class D103010Controller : CoreController
    {
        public D103010Controller(ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(viewEngine)
        {
        }
        ...
        public ActionResult Init()
        {
            ...
            if (!ScreenSosaUtil.CanReference(F103Const.SCREEN_ID_D103010, HttpContext))
            {
            ...
            //// モデル初期化
            D103010Model model = new D103010Model
            {
                // 「ログイン情報」を取得する
                VSyokuinRecords = getJigyoDb<NskAppContext>().VSyokuins.Where(t => t.UserId == Syokuin.UserId).Single()
            };
            var updateKengen = ScreenSosaUtil.CanUpdate(F103Const.SCREEN_ID_D103010, HttpContext);
            ...
            if (md != null)
            {
                model.D103010Info.SKyosaiMokutekiCd = md.SKyosaiMokutekiCd;
                model.D103010Info.SNensanHikiuke = md.SNensanHikiuke;
                model.D103010Info.SNensanHyoka = md.SNensanHyoka;
            ...
            // 初期表示情報をセッションに保存する
            SessionUtil.Set(F103Const.SESS_D103010, model, HttpContext);
            return View(F103Const.SCREEN_ID_D103010, model);
        }
        ...
        public ActionResult CreatBatchReport([Bind("TorikomiFilePath")] D103010Model form)
        {
            logger.Debug("START CreatBatchReport");
            var model = SessionUtil.Get<D103010Model>(F103Const.SESS_D103010, HttpContext);
            ...
            BatchUtil.GetBatchYoyakuListParam param = new()
            {
                ...
                BatchNm = F103Const.BATCH_ID_NSK_103011B
            };
            // 総件数取得フラグ
            ...
            {
                // メッセージ表示して処理を中止する
                return Json(new { success = false, message = MessageUtil.Get("ME10019", "基準統計単収取込処理") });
            }
            ...
            var modelVar = new D103010ModelVar
            {
                // ファイル名
            ...
            try
            {
                ...
                Syokuin.UserId,
                F103Const.SCREEN_ID_NSK_D103010,
                F103Const.BATCH_ID_NSK_103011B,
                strJoukenId,
                displayJouken,
                AppConst.FLG_OFF,
            ...
            return Json(new { success = true, message = MessageUtil.Get("MI00004", "基準統計単収取込バッチの予約") });
        }
        #endregion
        ...
        private string IsPrivateCheckError(D103010Model form)
        {
            ...
            try
            {
                ...
                // 【追加】拡張子とMIMEタイプのチェック
                if (fileExtension != F103Const.allowedExtension || uploadFile.ContentType != F103Const.allowedMimeType)
                {
                ...
                // ファイル名文字数をチェック
                if (fileName.Length > F103Const.fileNameMaxLength)
                {
                    errMessage = MessageUtil.Get("ME91008", $"{F103Const.fileNameMaxLength}");
                    ModelState.AddModelError("TorikomiFilePath", "");
                    return errMessage;
                }

                // アップロードするファイルサイズがuploadFileMaxSizeを超えている場合
                if (fileTotalSize > F103Const.uploadFileMaxSize)
                {
                    errMessage = MessageUtil.Get("ME10049");
                    ModelState.AddModelError("MessageArea", MessageUtil.Get("ME01421", F103Const.uploadFileMaxDispSize));
                    ModelState.AddModelError("TorikomiFilePath", "");
                    return errMessage;
                }
                // 外字チェック
        ...
        private void SetFormToModel(D103010Model form, D103010Model model)
        {
            // アップロードするファイル
        ...
        private string InsertTJouken(D103010Model model, string joukenId, string filePath, string fileHash)
        {
            // ユーザID
            var userId = Syokuin.UserId;
            // システム日時

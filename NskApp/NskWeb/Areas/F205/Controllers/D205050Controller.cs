...
using NskWeb.Areas.F205.Models.D205050;
...
{
    /// <summary>
    /// NSK_205050D_仮渡し金計算処理（インデックス）起動用画面
    /// </summary>
    /// <remarks>
    /// 作成日：2025/04/10
    /// 作成者：ネクスト松嶋
    ...
    [AllowAnonymous]
    [Area("F205")]
    public class D205050Controller : CoreController
    {
        public D205050Controller(ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(viewEngine)
        {
        }
...

        {
            ...
            if (!ScreenSosaUtil.CanReference(F205Const.SCREEN_ID_D205050, HttpContext))
            {
            ...
            //// モデル初期化
            D205050Model model = new D205050Model(Syokuin, ShishoList)
            {
                // 「ログイン情報」を取得する
                VSyokuinRecords = getJigyoDb<NskAppContext>().VSyokuins.Where(t => t.UserId == Syokuin.UserId).Single()
            };

            var updateKengen = ScreenSosaUtil.CanUpdate(F205Const.SCREEN_ID_D205050, HttpContext);
            ...
            // 初期表示情報をセッションに保存する
            SessionUtil.Set(F205Const.SESS_D205050, model, HttpContext);
            return View(F205Const.SCREEN_ID_D205050, model);
        }
        ...
        public ActionResult CreatBatchReport([Bind("ShishoCd")] D205050Model form)
        {
            logger.Debug("START CreatBatchReport");
            var selectedShisho = form.ShishoCd;
            var model = SessionUtil.Get<D205050Model>(F205Const.SESS_D205050, HttpContext);
            ...
            BatchUtil.GetBatchYoyakuListParam param = new()
            {
                ...
                BatchNm = F205Const.BATCH_ID_NSK_205051B
            };
            // 総件数取得フラグ
            ...
            try
            {
                ...
                Syokuin.UserId,
                F205Const.SCREEN_ID_NSK_D205050,
                F205Const.BATCH_ID_NSK_205051B,
                strJoukenId,
                displayJouken,
                AppConst.FLG_OFF,
            ...
            return Json(new { success = true, message = MessageUtil.Get("MI00004", "仮渡し金計算処理（インデックス）") });
        }
        #endregion

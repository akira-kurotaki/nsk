...
using NskWeb.Areas.F205.Models.D205020;
...
{
    /// <summary>
    /// NSK_205020D_仮渡し金計算処理（全相殺）起動用画面
    /// </summary>
    /// <remarks>
    /// 作成日：2025/04/11
    /// 作成者：ネクスト松嶋
    ...
    [AllowAnonymous]
    [Area("F205")]
    public class D205020Controller : CoreController
    {
        public D205020Controller(ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(viewEngine)
        {
        }
...

        {
            ...
            if (!ScreenSosaUtil.CanReference(F205Const.SCREEN_ID_D205020, HttpContext))
            {
            ...
            //// モデル初期化
            D205020Model model = new D205020Model(Syokuin, ShishoList)
            {
                // 「ログイン情報」を取得する
                VSyokuinRecords = getJigyoDb<NskAppContext>().VSyokuins.Where(t => t.UserId == Syokuin.UserId).Single()
            };

            var updateKengen = ScreenSosaUtil.CanUpdate(F205Const.SCREEN_ID_D205020, HttpContext);
            ...
            if (md != null)
            {
                model.D205020Info.SKyosaiMokutekiCd = md.SKyosaiMokutekiCd;
                model.D205020Info.SNensanHyoka = md.SNensanHyoka;
            ...
            // 初期表示情報をセッションに保存する
            SessionUtil.Set(F205Const.SESS_D205020, model, HttpContext);
            return View(F205Const.SCREEN_ID_D205020, model);
        }
        ...
        public ActionResult CreatBatchReport([Bind("ShishoCd,Taishosha")] D205020Model form)
        {
            logger.Debug("START CreatBatchReport");
            var selectedShisho = form.ShishoCd;
            var model = SessionUtil.Get<D205020Model>(F205Const.SESS_D205020, HttpContext);
            ...
            // 画面入力された「対象者選択」の値をセッションモデルに反映
            model.Taishosha = form.Taishosha;
            // 総件数取得フラグ
                        ...
                        .ToList() is var waitingList && waitingList.Any())
            {
                using (var db1 = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema))
                {
                    ...
                    if (hasMatching)
                    {
                        ...
                        {
                            success = false,
                            message = MessageUtil.Get("ME10019", "仮渡し金計算処理（全相殺）")
                        });
                    }
                }
            }
            ...
            long batchId = 0;
            // バッチ条件（表示用）作成
            var displayJouken = NskConsts.JoukenNameConst.JOUKEN_NENSAN + "、"  + NskConsts.JoukenNameConst.JOUKEN_SHISHO + "、" + NskConsts.JoukenNameConst.JOUKEN_KYOSAI_MOKUTEKI_CD + "、" + "仮渡し金計算処理対象者"; // TODO: 要条件定数追加 NskConsts.JoukenNameConst.JOUKEN_KARIWATASI_KEISAN_TAISYOSYA
            ...
            try
            {
                ...
                Syokuin.UserId,
                F205Const.SCREEN_ID_NSK_D205020,
                F205Const.BATCH_ID_NSK_205021B,
                strJoukenId,
                displayJouken,
                AppConst.FLG_OFF,
                ...
                ref refMsg,
                ref batchId,
                F205Const.SCREEN_ID_NSK_D205020 + Syokuin.TodofukenCd + selectedShisho
                );
            }
            catch (Exception e)
            {
            ...
            return Json(new { success = true, message = MessageUtil.Get("MI00004", "仮渡し金計算処理（全相殺）バッチの予約") });
        }
        #endregion
        ...
        private string InsertTJouken(DbConnectionInfo dbConnectionInfo, D205020Model model, string joukenId,string shishocd, NSKPortalInfoModel md)
        {
            // ユーザID
            var userId = Syokuin.UserId;
            // システム日時
            ...
            using (var db1 = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema))
            {
                ...
                try
                {
                    ...
                    // 対象者選択（青色申告組合員／一筆全損半損被害耕地）の選択値をバッチ条件としてDBに登録
                    var taishosha = new T01050バッチ条件
                    {
                        バッチ条件id = joukenId,
                        連番 = ++serialNumber,
                        条件名称 = "仮渡し金計算処理対象者",　　　// TODO: 要条件定数追加 NskConsts.JoukenNameConst.JOUKEN_KARIWATASI_KEISAN_TAISYOSYA
                        表示用条件値 = "仮渡し金計算処理対象者",　// TODO: 要条件定数追加 NskConsts.JoukenNameConst.JOUKEN_KARIWATASI_KEISAN_TAISYOSYA
                        条件値 = model.Taishosha, // 0 or 1
                        登録日時 = systemDate,
                        登録ユーザid = userId,
                        更新日時 = systemDate,
                        更新ユーザid = userId
                    };
                    db1.T01050バッチ条件s.Add(taishosha);

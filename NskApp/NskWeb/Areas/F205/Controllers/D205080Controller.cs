...
using NskWeb.Areas.F205.Models.D205080;
...
{
    /// <summary>
    /// NSK_205080D_仮渡し金振込消込み（インターフェース）
    /// </summary>
    /// <remarks>
    /// 作成日：2025/04/15
    /// 作成者：ネクスト松嶋
    ...
    [AllowAnonymous]
    [Area("F205")]
    public class D205080Controller : CoreController
    {
        public D205080Controller(ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(viewEngine)
        {
        }
...

        {
            ...
            if (!ScreenSosaUtil.CanReference(F205Const.SCREEN_ID_D205080, HttpContext))
            {
            ...
            //// モデル初期化
            D205080Model model = new D205080Model(Syokuin, ShishoList)
            {
                // 「ログイン情報」を取得する
                VSyokuinRecords = getJigyoDb<NskAppContext>().VSyokuins.Where(t => t.UserId == Syokuin.UserId).Single()
            };

            var updateKengen = ScreenSosaUtil.CanUpdate(F205Const.SCREEN_ID_D205080, HttpContext);
            ...
            if (md != null)
            {
                model.D205080Info.SKyosaiMokutekiCd = md.SKyosaiMokutekiCd;
                model.D205080Info.SNensanHyoka = md.SNensanHyoka;
            ...
            // 初期表示情報をセッションに保存する
            SessionUtil.Set(F205Const.SESS_D205080, model, HttpContext);
            return View(F205Const.SCREEN_ID_D205080, model);
        }
        ...
        public ActionResult CreatBatchReport([Bind("ShishoCd,TaishoFurikaeDate")] D205080Model form)
        {
            logger.Debug("START CreatBatchReport");
            var selectedShisho = form.ShishoCd;
            var model = SessionUtil.Get<D205080Model>(F205Const.SESS_D205080, HttpContext);
            ...
            //// 画面から対象データ振替日が取得できなかった場合
            if (form.TaishoFurikaeDate == null)
            {
                return Json(new { success = false, message = MessageUtil.Get("ME00001", "対象データ振替日", "") });
            }
            ...
            BatchUtil.GetBatchYoyakuListParam param = new()
            {
                ...
                BatchNm = F205Const.BATCH_ID_NSK_205081B
            };
            // 画面入力された「対象データ振替日」の値をセッションモデルに反映
            model.TaishoFurikaeDate = form.TaishoFurikaeDate;
            // 総件数取得フラグ
                        ...
                        .ToList() is var waitingList && waitingList.Any())
            {
                using (var db1 = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema))
                {
                    ...
                    if (hasMatching)
                    {
                        ...
                        {
                            success = false,
                            message = MessageUtil.Get("ME10019", " 仮渡し金振込消込み（インターフェース）")
                        });
                    }
                }
            }
            ...
            long batchId = 0;
            // バッチ条件（表示用）作成
            var displayJouken = NskConsts.JoukenNameConst.JOUKEN_NENSAN + "、"  + NskConsts.JoukenNameConst.JOUKEN_SHISHO + "、" + NskConsts.JoukenNameConst.JOUKEN_TAISHO_FURIKAE_DATE; 
            ...
            try
            {
                ...
                Syokuin.UserId,
                F205Const.SCREEN_ID_NSK_D205080,
                F205Const.BATCH_ID_NSK_205081B,
                strJoukenId,
                displayJouken,
                AppConst.FLG_OFF,
                ...
                ref refMsg,
                ref batchId,
                F205Const.SCREEN_ID_NSK_D205080 + Syokuin.TodofukenCd + selectedShisho
                );
            }
            catch (Exception e)
            {
            ...
            return Json(new { success = true, message = MessageUtil.Get("MI00004", "仮渡し金振込消込み（インターフェース）バッチの予約") });
        }
        #endregion
        ...
        private string InsertTJouken(DbConnectionInfo dbConnectionInfo, D205080Model model, string joukenId,string shishocd, NSKPortalInfoModel md)
        {
            // ユーザID
            var userId = Syokuin.UserId;
            // システム日時
            ...
            using (var db1 = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema))
            {
                ...
                try
                {
                    ...
                    var taishoFurikaeDate = new T01050バッチ条件
                    {
                        バッチ条件id = joukenId,
                        連番 = ++serialNumber,
                        条件名称 = JoukenNameConst.JOUKEN_TAISHO_FURIKAE_DATE,
                        表示用条件値 = JoukenNameConst.JOUKEN_TAISHO_FURIKAE_DATE,
                        条件値 = model.TaishoFurikaeDate.ToString(),
                        登録日時 = systemDate,
                        登録ユーザid = userId,
                        更新日時 = systemDate,
                        更新ユーザid = userId
                    };
                    db1.T01050バッチ条件s.Add(taishoFurikaeDate);

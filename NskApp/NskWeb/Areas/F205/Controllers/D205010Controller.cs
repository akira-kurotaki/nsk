...
using NskWeb.Areas.F205.Models.D205010;
...
{
    /// <summary>
    /// NSK_205010D_仮渡し金計算処理（半相殺）起動用画面
    /// </summary>
    /// <remarks>
    /// 作成日：2025/04/10
    /// 作成者：ネクスト松嶋
    ...
    [AllowAnonymous]
    [Area("F205")]
    public class D205010Controller : CoreController
    {
        public D205010Controller(ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(viewEngine)
        {
        }
...

        {
            ...
            if (!ScreenSosaUtil.CanReference(F205Const.SCREEN_ID_D205010, HttpContext))
            {
            ...
            //// モデル初期化
            D205010Model model = new D205010Model(Syokuin, ShishoList)
            {
                // 「ログイン情報」を取得する
                VSyokuinRecords = getJigyoDb<NskAppContext>().VSyokuins.Where(t => t.UserId == Syokuin.UserId).Single()
            };

            var updateKengen = ScreenSosaUtil.CanUpdate(F205Const.SCREEN_ID_D205010, HttpContext);
            ...
            if (md != null)
            {
                model.D205010Info.SKyosaiMokutekiCd = md.SKyosaiMokutekiCd;
                model.D205010Info.SNensanHyoka = md.SNensanHyoka;
            ...
            // 初期表示情報をセッションに保存する
            SessionUtil.Set(F205Const.SESS_D205010, model, HttpContext);
            return View(F205Const.SCREEN_ID_D205010, model);
        }
        ...
        public ActionResult CreatBatchReport([Bind("ShishoCd")] D205010Model form)
        {
            logger.Debug("START CreatBatchReport");
            var selectedShisho = form.ShishoCd;
            var model = SessionUtil.Get<D205010Model>(F205Const.SESS_D205010, HttpContext);
            ...
            BatchUtil.GetBatchYoyakuListParam param = new()
            {
                ...
                BatchNm = F205Const.BATCH_ID_NSK_205011B
            };
            // 総件数取得フラグ
                        ...
                        .ToList() is var waitingList && waitingList.Any())
            {
                using (var db1 = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema))
                {
                    ...
                    if (hasMatching)
                    {
                        ...
                        {
                            success = false,
                            message = MessageUtil.Get("ME10019", "仮渡し金計算処理（半相殺）")
                        });
                    }
                }
            }
            ...
            long batchId = 0;
            // バッチ条件（表示用）作成
            var displayJouken = NskConsts.JoukenNameConst.JOUKEN_NENSAN + "、"  + NskConsts.JoukenNameConst.JOUKEN_SHISHO + "、" + NskConsts.JoukenNameConst.JOUKEN_KYOSAI_MOKUTEKI_CD; 
            ...
            try
            {
                ...
                Syokuin.UserId,
                F205Const.SCREEN_ID_NSK_D205010,
                F205Const.BATCH_ID_NSK_205011B,
                strJoukenId,
                displayJouken,
                AppConst.FLG_OFF,
                ...
                ref refMsg,
                ref batchId,
                F205Const.SCREEN_ID_NSK_D205010 + Syokuin.TodofukenCd + selectedShisho
                );
            }
            catch (Exception e)
            {
            ...
            return Json(new { success = true, message = MessageUtil.Get("MI00004", "仮渡し金計算処理（半相殺）バッチの予約") });
        }
        #endregion
        ...
        private string InsertTJouken(DbConnectionInfo dbConnectionInfo, D205010Model model, string joukenId,string shishocd, NSKPortalInfoModel md)
        {
            // ユーザID
            var userId = Syokuin.UserId;
            // システム日時

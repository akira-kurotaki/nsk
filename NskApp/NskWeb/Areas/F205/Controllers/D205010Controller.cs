    ...
    public class D205010Controller : CoreController
    {
...

        {
            ...
            if (md != null)
            {
                ...
                model.D205010Info.SNensanHikiuke = md.SNensanHikiuke;
        ...
        public ActionResult CreatBatchReport([Bind("ShishoCd")] D205010Model form)
        {
                        ...
                        .ToList() is var waitingList && waitingList.Any())
            {
                using (var db1 = new NskAppContext(dbConnectionInfo.ConnectionString, dbConnectionInfo.DefaultSchema))
                {
                    ...
                    if (hasMatching)
                    {
                        ...
                        {
                            success = false,
                            message = MessageUtil.Get("ME10019", "選択された支所の仮渡し金計算処理（半相殺）")
                        });
                    }
                }
            }
            ...
            try
            {
                ...
                ref refMsg,
                ref batchId,
                F205Const.SCREEN_ID_NSK_D205010 + Syokuin.TodofukenCd
                );
            }
            catch (Exception e)
            {
        ...
        private string InsertTJouken(DbConnectionInfo dbConnectionInfo, D205010Model model, string joukenId,string shishocd)
        {
            // ユーザID
            var userId = Syokuin.UserId;
            // システム日時

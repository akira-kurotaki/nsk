...
using NskWeb.Areas.F102.Models.D102100;
...
{
    /// <summary>
    /// NSK_102100D_危険段階データ取込（組合員等別危険段階区分）（テキスト）
    /// </summary>
    /// <remarks>
    /// 作成日：2025/01/08
    /// 作成者：ジョンジュンヒョ
    ...
    [AllowAnonymous]
    [Area("F102")]
    public class D102100Controller : CoreController
    {
        #region メンバー定数
        /// <summary>
        /// 画面ID(D2010)
        /// </summary>
        private static readonly string SCREEN_ID_D102100 = "D102100";

        /// <summary>
        /// セッションキー(D102100)
        /// </summary>
        private static readonly string SESS_D102100 = "D102100_SCREEN";
        /// <summary>
        /// 予約を実行した処理名
        /// </summary>
        private static readonly string D102100_REPORT_YOYAKU_SHORI_NM = "NSK_102100D";
        /// <summary>
        /// バッチ名
        /// </summary>
        private static readonly string D102100_BATCH_NM = "危険段階データ取込（組合員等別危険段階区分）（テキスト）";

        private readonly IWebHostEnvironment _webHostEnvironment;

        public D102100Controller(ICompositeViewEngine viewEngine, IWebHostEnvironment webHostEnvironment) : base(viewEngine)
        {
        ...
        public ActionResult Init()
        {
            ...
            if (!ScreenSosaUtil.CanReference(SCREEN_ID_D102100, HttpContext))
            {
            ...
            //// モデル初期化
            D102100Model model = new D102100Model
            {
                // 「ログイン情報」を取得する
                VSyokuinRecords = getJigyoDb<NskAppContext>().VSyokuins.Where(t => t.UserId == Syokuin.UserId).Single()
            };
            var updateKengen = ScreenSosaUtil.CanUpdate(SCREEN_ID_D102100, HttpContext);
            ...
            if (md != null)
            {
                model.D102100Info.SKyosaiMokutekiCd = md.SKyosaiMokutekiCd;
                model.D102100Info.SNensanHikiuke = md.SNensanHikiuke;
            ...
            // 初期表示情報をセッションに保存する
            SessionUtil.Set(SESS_D102100, model, HttpContext);
            return View(SCREEN_ID_D102100, model);
        }
        ...
        public ActionResult CreatBatchReport([Bind("TorikomiFilePath")] D102100Model form)
        {
            logger.Debug("START CreatBatchReport");
            var model = SessionUtil.Get<D102100Model>(SESS_D102100, HttpContext);
            ...
            var modelVar = new D102100ModelVar
            {
                // ファイル名
            ...
            try
            {
                ...
                Syokuin.UserId,
                D102100_REPORT_YOYAKU_SHORI_NM,
                D102100_BATCH_NM,
                strJoukenId,
                displayJouken,
                AppConst.FLG_OFF,
        ...
        private string IsPrivateCheckError(D102100Model form)
        {
            ...
            // 許容ファイルサイズ（表示用）
            var uploadFileMaxDispSize = "10MB"; // TODO: aasettings.json にファイルサイズ（表示用）が追加されたら置き換える
        ...
        private void SetFormToModel(D102100Model form, D102100Model model)
        {
            // アップロードするファイル
        ...
        private string InsertTJouken(D102100Model model, string joukenId, string filePath, string fileHash)
        {
            // ユーザID
            var userId = Syokuin.UserId;
            // システム日時

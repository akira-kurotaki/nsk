...
namespace NskWeb.Areas.F105.Models.D105070
{
    ...
    public class D105070HikiukeSearchResult : BasePager<D105070HikiukeRecord>
    {
        ...
        /// コンストラクタ
        /// </summary>
        public D105070HikiukeSearchResult()
        {
            SearchCondition = new();
        }
        ...
        public D105070HikiukeSearchResult(D105070SearchCondition searchCondition)
        {
        ...
        public override List<D105070HikiukeRecord> GetResult(NskAppContext dbContext, BaseSessionInfo session)
        {
            ...
            query.Append(" SELECT ");
            query.Append($"   T1.耕地番号         As \"{nameof(D105070HikiukeRecord.KouchiNo)}\" ");
            query.Append($"  ,T1.地名地番         As \"{nameof(D105070HikiukeRecord.ChimeiChiban)}\" ");
            query.Append($"  ,T1.耕地面積         As \"{nameof(D105070HikiukeRecord.KouchiMenseki)}\" ");
            query.Append($"  ,T1.引受面積         As \"{nameof(D105070HikiukeRecord.HikukeMenseki)}\" ");
            query.Append($"  ,T1.転作等面積       As \"{nameof(D105070HikiukeRecord.JogaitoMenseki)}\" ");
            query.Append($"  ,T1.種類コード       As \"{nameof(D105070HikiukeRecord.Syurui)}\" ");
            query.Append($"  ,T1.区分コード       As \"{nameof(D105070HikiukeRecord.Kbn)}\" ");
            query.Append($"  ,T1.統計市町村コード As \"{nameof(D105070HikiukeRecord.Shichoson)}\" ");
            query.Append($"  ,M3.shichoson_nm     As \"{nameof(D105070HikiukeRecord.ShichosonNm)}\" ");
            query.Append($"  ,T1.品種コード       As \"{nameof(D105070HikiukeRecord.Hinsyu)}\" ");
            query.Append($"  ,M1.品種名等         As \"{nameof(D105070HikiukeRecord.HinsyuNm)}\" ");
            query.Append($"  ,T1.産地別銘柄コード As \"{nameof(D105070HikiukeRecord.SanchiMeigara)}\" ");
            query.Append($"  ,M2.産地別銘柄名称   As \"{nameof(D105070HikiukeRecord.SanchiMeigaraNm)}\" ");
            query.Append($"  ,T1.田畑区分         As \"{nameof(D105070HikiukeRecord.Tahata)}\" ");
            query.Append($"  ,T1.収量等級コード   As \"{nameof(D105070HikiukeRecord.SyuryoTokyu)}\" ");
            query.Append($"  ,T1.分筆番号         As \"{nameof(D105070HikiukeRecord.BunpitsuNo)}\" ");
            query.Append($"  ,T1.統計単収         As \"{nameof(D105070HikiukeRecord.ToukeiTansyu)}\" ");
            query.Append($"  ,T1.実量基準単収     As \"{nameof(D105070HikiukeRecord.JituryoKijunTansyu)}\" ");
            query.Append($"  ,T1.参酌コード       As \"{nameof(D105070HikiukeRecord.Sanjaku)}\" ");
            query.Append($"  ,T1.受委託区分       As \"{nameof(D105070HikiukeRecord.JuitakushaKbn)}\" ");
            query.Append($"  ,T1.受委託者コード   As \"{nameof(D105070HikiukeRecord.Shoyusha)}\" ");
            query.Append($"  ,T1.備考             As \"{nameof(D105070HikiukeRecord.Bikou)}\" ");
            query.Append($"  ,T2.局都道府県コード As \"{nameof(D105070HikiukeRecord.GisKyokuTodofuken)}\" ");
            query.Append($"  ,T2.市区町村コード   As \"{nameof(D105070HikiukeRecord.GisShichoson)}\" ");
            query.Append($"  ,T2.大字コード       As \"{nameof(D105070HikiukeRecord.GisOoaza)}\" ");
            query.Append($"  ,T2.小字コード       As \"{nameof(D105070HikiukeRecord.GisKoaza)}\" ");
            query.Append($"  ,T2.地番             As \"{nameof(D105070HikiukeRecord.GisChiban)}\" ");
            query.Append($"  ,T2.枝番             As \"{nameof(D105070HikiukeRecord.GisEdaban)}\" ");
            query.Append($"  ,T2.子番             As \"{nameof(D105070HikiukeRecord.GisKoban)}\" ");
            query.Append($"  ,T2.孫番             As \"{nameof(D105070HikiukeRecord.GisMagoban)}\" ");
            query.Append($"  ,T2.RS区分           As \"{nameof(D105070HikiukeRecord.GisRsKbn)}\" ");
            query.Append($"  ,cast('' || T1.xmin as integer) As \"{nameof(D105070HikiukeRecord.KouchiXmin)}\" ");
            query.Append($"  ,cast('' || T2.xmin as integer) As \"{nameof(D105070HikiukeRecord.GisXmin)}\" ");
            ...
            if (SearchCondition.DisplaySort1.HasValue || SearchCondition.DisplaySort2.HasValue)
            {
                ...
                if (SearchCondition.DisplaySort1.HasValue)
                {
                    ...
                    {
                        case D105070SearchCondition.DisplaySortType.Chiban:
                            query.Append($" T1.{nameof(T11090引受耕地.地名地番)} {SearchCondition.DisplaySortOrder1} ");
                            break;
                        case D105070SearchCondition.DisplaySortType.KouchBango:
            ...
            List<D105070HikiukeRecord> records = new();
            records.AddRange(dbContext.Database.SqlQueryRaw<D105070HikiukeRecord>(query.ToString(), queryParams.ToArray()));
        ...
        public int DeleteHikiukeGis(ref NskAppContext dbContext, 
            D105070SessionInfo sessionInfo, 
            ref List<D105070HikiukeRecord> hikiukeDelRecords)
        {
            int delCount = 0;
            ...
            foreach (D105070HikiukeRecord target in hikiukeDelRecords)
            {
        ...
        public int UpdateHikiukeGis(ref NskAppContext dbContext,
            ...
            string userId,
            DateTime? sysDateTime,
            ref List<D105070HikiukeRecord> hikiukeUpdRecords)
        {
            int updCount = 0;
            ...
            foreach (D105070HikiukeRecord target in hikiukeUpdRecords)
            {
                if (!target.KouchiXmin.HasValue)
                {
                    // xmin nullは処理対象外
                ...
                [
                    ...
                    new("分筆番号", target.BunpitsuNo),
                    new("地名地番", string.IsNullOrEmpty(target.ChimeiChiban) ? DBNull.Value : target.ChimeiChiban),
                    new("耕地面積", target.KouchiMenseki.HasValue ? target.KouchiMenseki : DBNull.Value),
                    new("引受面積", target.HikukeMenseki.HasValue ? target.HikukeMenseki : DBNull.Value),
                    new("転作等面積", target.JogaitoMenseki.HasValue ? target.JogaitoMenseki : DBNull.Value),
        ...
        public int AppendHikiukeGis(ref NskAppContext dbContext,
            ...
            string userId,
            DateTime? sysDateTime,
             ref List<D105070HikiukeRecord> hikiukeAddRecords)
        {
            int addCount = 0;

            // t_11090_引受耕地の対象レコードを登録する。
            List<T11090引受耕地> addHikiukeRecs = new();
            foreach (D105070HikiukeRecord target in hikiukeAddRecords)
            {
                addHikiukeRecs.Add(new()
                {
                    ...
                    耕地面積 = target.KouchiMenseki,
                    引受面積 = target.HikukeMenseki,
                    転作等面積 = target.JogaitoMenseki,
        ...
        public void ApplyInput(D105070HikiukeSearchResult src)
        {
        ...
        /// <param name="sessionInfo"></param>
        /// <returns></returns>
        public override List<D105070HikiukeRecord> GetUpdateRecs(ref NskAppContext dbContext, BaseSessionInfo sessionInfo)
        {
            List<D105070HikiukeRecord> updRecs = new();

            // 検索結果取得
            List<D105070HikiukeRecord> dbResults = GetResult(dbContext, sessionInfo);

            // 検索結果と画面入力値を比較
            foreach (D105070HikiukeRecord dispRec in DispRecords)
            {
                // 追加行、削除行以外を対象とする
                if (dispRec is BasePagerRecord pagerRec && !pagerRec.IsNewRec && !pagerRec.IsDelRec)
                {
                    D105070HikiukeRecord dbRec = dbResults.SingleOrDefault(x =>

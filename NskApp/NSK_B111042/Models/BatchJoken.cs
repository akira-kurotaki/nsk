...
namespace NSK_B111042.Models
{
    ...
    public class BatchJoken
    {
        ...
        public static List<KoufuKinShinseiRecord> GetKoufuKinShinsei(NskAppContext dbContext, string todofukenCd, string kumiaitoCd, BatchJoken batchJoken)
        {
            ...
            sql.Append($@"	,COALESCE(T1.組合等別国庫負担金, 0) - COALESCE(T1.保険料総額, 0) AS 組合等負担金交付対象金額 ");
            sql.Append($@"	,T1.組合員等負担共済掛金 ");
            sql.Append($@"	,T1.組合員等負担共済掛金徴収済額 ");
            sql.Append($@"	,T1.共済掛金徴収割合 ");
            sql.Append($@"	,T1.組合等交付金の金額 ");
            sql.Append($@"	,COALESCE(T2.組合等交付金の金額, 0) AS 既受領交付金の金額 ");
            sql.Append($@"	,COALESCE(T1.組合等交付金の金額, 0) - COALESCE(T2.組合等交付金の金額, 0) AS 今回交付申請額 ");
            sql.Append($@"	,T1.年産 ");
            sql.Append($@"FROM ");
            sql.Append($@"		t_15020_組合等交付 T1 ");
            sql.Append($@"LEFT OUTER JOIN t_15020_組合等交付 T2 ");
            sql.Append($@"	ON T1.組合等コード = T2.組合等コード ");
            sql.Append($@"	AND T1.負担金交付区分 = T2.負担金交付区分 ");
            sql.Append($@"	AND T1.年産 = T2.年産 ");
            sql.Append($@"	AND T1.交付回 >= T2.交付回 ");
            sql.Append($@"	AND T2.交付回 = @条件_交付回 - 1 ");
            sql.Append($@"INNER JOIN v_kumiaito T3 ");
            sql.Append($@"	ON T3.todofuken_cd = @都道府県コード ");
            sql.Append($@"	AND T1.組合等コード = T3.kumiaito_cd ");
            sql.Append($@"WHERE ");

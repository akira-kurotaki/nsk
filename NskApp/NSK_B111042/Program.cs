        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (NskAppContext dbContext = new(dbInfo.ConnectionString, dbInfo.DefaultSchema, ConfigUtil.GetInt("CommandTimeout")))
                {
                    ...
                    // ファイル名："360" & [変数：条件_負担金交付区分] & "01" & [変数：条件_組合等コード] & ".TXT"
                    string fileName = "360X01YY".Replace("X", batchJoken.JokenFutankinKofuKbn).Replace("YY", batchJoken.JokenKumiaitoCd) + NskCommon.CoreConst.FILE_EXTENSION_TXT;
                    string filePath = Path.Combine(tempFolderPath, fileName);

                    // ８．２．取得した組合等交付金申請書データを組合等交付金申請書データファイルにカンマ区切りで出力
                    // ファイル設計書に沿って交付金申請書データ作成_組合等交付金申請ファイルに交付金申請書データを出力する。
                        ...
                        using (StreamWriter writer = new(fs, encoding))
                        {
                            ...
                            foreach (KoufuKinShinseiRecord koufuKinShinseiRecord in koufuKinShinsei)
                            {
                                ...
                                [
                                    ...
                                    // 組合等別連合会保険料
                                    koufuKinShinseiRecord.保険料総額.ToString(),
                                    // 組合等別国庫負担金
                                    koufuKinShinseiRecord.組合等別国庫負担金.ToString(),
                                    // 組合等負担金交付対象金額
                                    koufuKinShinseiRecord.組合等負担金交付対象金額.ToString(),
                                    // 組合員等負担共済掛金

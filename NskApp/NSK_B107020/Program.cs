        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                using (NskAppContext dbContext = new(dbInfo.ConnectionString, dbInfo.DefaultSchema, ConfigUtil.GetInt(Core.CoreConst.COMMAND_TIMEOUT)))
                {
                    ...
                    // ６．帳票を作成する。
                    using (var reportController = new NskReportP107020Controller(dbInfo))
                    {
                        ControllerResult controllerResult = null;
                        // ６．１．「帳票制御処理_NSK_107020P_耕地等情報（６号）」を呼び出す。
                        controllerResult = reportController.ManageReports("NSK_107020P", jid, todofukenCd, kumiaitoCd, shishoCd, nBid);
